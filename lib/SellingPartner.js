...
class SellingPartner {
  ...
  constructor(config) {
    this._axiosClient = config.axiosClient;
    ...
    if (!this._refresh_token && !this._options.only_grantless_operations) {
      throw new CustomError({
        ...
        message:
          'Please provide a refresh token or set "only_grantless_operations" option to true',
      });
    }
  }
  ...
  _validateEndpointsVersions(endpoints_versions) {
    let invalid_endpoints = Object.keys(endpoints_versions).filter(
      (endpoint) => {
    ...
    if (invalid_endpoints.length) {
      throw new CustomError({
        ...
        message:
          "One or more endpoints are not valid. These endpoints don't exist: \"" +
          invalid_endpoints.join('","') +
          '"',
      });
    }
    let invalid_endpoints_versions = Object.keys(endpoints_versions).filter(
      (endpoint) => {
        return !endpoints[endpoint].__versions.includes(
          endpoints_versions[endpoint],
        );
      },
    );
  ...
  _constructRefreshAccessTokenBody(scope) {
    ...
    if (scope) {
      ...
      throw new CustomError({
        ...
        message:
          "Grantless tokens require a scope. Please provide one of: " +
          valid_scopes.join(","),
      });
    }
  ...
  _tokenExists(scope) {
    return (
      (this._access_token && !scope) || (scope && this._grantless_tokens[scope])
    );
  }
  ...
  _validateMethod(method) {
    if (
      !method ||
      !/^(GET|POST|PUT|DELETE|PATCH)$/.test(method.toUpperCase())
    ) {
      throw new CustomError({
        code: "NO_VALID_METHOD_PROVIDED",
        message:
          'Please provide a valid HTTP Method ("GET","POST","PUT","DELETE" or "PATCH") when using "api_path"',
      });
    }
  ...
  async refreshRoleCredentials() {
    ...
    if (
      ...
      json_res &&
      json_res.ErrorResponse &&
      json_res.ErrorResponse.Error
    ) {
      throw new CustomError({
  ...
  async callAPI(req_params) {
    ...
    if (req_params.api_path) {
      ...
      let { operation, endpoint } = this._validateOperationAndEndpoint(
        req_params.operation,
        req_params.endpoint,
      );
      let version = this._validateAndGetVersion(
        operation,
        endpoint,
        options.version,
      );
    ...
    let signed_request = new Signer(
      ...
      this._options.use_sandbox,
      this._options.user_agent,
    ).signAPIRequest(token_for_request, this._role_credentials, req_params);
    let res = await request({
      ...signed_request,
      axiosClient: this.axiosClient,
    });
      ...
      if (res.statusCode === 403 && error.code === "Unauthorized") {
        if (this._options.auto_request_tokens) {
          if (/access token.*expired/.test(error.details)) {
            ...
            if (this._options.debug_log) {
              console.log(
                "Role credentials security token expired, refreshing it now",
              );
            }
            await this.refreshRoleCredentials();
            return await this.callAPI(req_params);
    ...
    const response = json_res.payload || json_res;
    if (response) {
      return Object.defineProperties(json_res.payload || json_res, {
        rutter: {
          writable: false,
          enumerable: false,
          value: {
  ...
  async download(details, options = {}) {
    ...
    if (details.encryptionDetails) {
      ...
      decrypted_buffer = Buffer.concat([
  ...
  async upload(details, feed) {
    ...
    let feed_content =
      feed.content || (await this._readFile(feed.file, feed.contentType));
    let content_buffer;
    if (details.encryptionDetails) {
